...
public class FileUpdater {
    ...
    public static void checkConfig(){
        if(Main.getInst().getConfig().get("plotLimits.default") == null){
            ...
            logger.log(Level.WARNING, "[Updater] Old config saved to " + old.getAbsolutePath() + ".old.");
        }
    }
    // Version: 1.1
    public static void checkMessages(){
        if(Messages.messages.get("messages.reloadSuccess") == null){
            ...
            logger.log(Level.WARNING, "[Updater] Old messages saved to " + old.getAbsolutePath() + ".old.");
        }
    }
}
